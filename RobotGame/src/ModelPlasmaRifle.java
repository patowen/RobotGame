...
/**
 * This class holds static methods that initialize and draw the plasma rifle model,
...
public class ModelPlasmaRifle
{
	...
	private static void storePointHook(double ttheta, double htheta, GL2 gl)
	{
		...
		double big = hbigrad *(1+ttheta/Math.PI);
		big = hbigrad;
		float x = (float)((big+small*Math.cos(htheta))*Math.cos(ttheta));
		float y = (float)((big+small*Math.cos(htheta))*Math.sin(ttheta));
		...
		float xnorm = (float)(-2*(big - temp)*x/temp);
		float ynorm = (float)(-2*(big - temp)*y/temp);
		float znorm = 2*z;
